...
const getParams = () => {
    ...
    const handler = (appId) => {
        ...
        if (!APP_IDS.includes(CACHE_APP_ID)) {
            ...
            $.msg($.name, 'Success', `Captured and Stored APP_ID: ${appId}`)
        } else {
            $.msg($.name, '', `APP_ID: ${appId} Already Exists`)
        }
    }
    // 打开TF APP抓取的信息参数
    ...
    // 打开链接需要抓取的参数
        ...
        if (appIdMatch && appIdMatch[1]) {
            ...
            $.log('No valid 𝐓𝐞𝐬𝐭𝐅𝐥𝐢𝐠𝐡𝐭 was not captured APP_ID')
        }
...
const TF_Check = (app_id) => {
    return new Promise((resolve, reject) => {
        $.get({ url: baseURL + app_id, headers }, (error, response, data) => {
            if (error) {
                return reject(`${app_id} Network request failed: ${error}`)
            }
            if (response.status !== 200) {
                APP_IDS.splice(inArray(app_id), 1)
                $.setdata(APP_IDS.join(','), 'tf_app_ids')
                $.msg('Not a valid 𝐓𝐞𝐬𝐭𝐅𝐥𝐢𝐠𝐡𝐭 link', '', `${app_id} has been removed`)
                return reject(`${app_id} is not a valid link: status code ${response.status}，Remove APP_ID`)
            }
            const appData = $.toObj(data)
            if (!appData) {
                return reject(`${app_id} Data parsing failed: ${data}`)
            }
            resolve(appData)
        })
    })
        ...
        $.post(
            ...
            (error, response, data) => {
                if (!error && response.status === 200) {
                    ...
                    if (!jsonBody) {
                        return reject(`${app_id} Join request response parsing failed: ${data}`)
                    }
                    resolve(jsonBody)
                } else {
                    reject(`${app_id} Failed to join: ${error || `status code ${response.status}`}`)
                }
            }
        )
    })
...
;(async () => {
    ...
    if (!Key || !SessionId || !SessionDigest || !RequestId) return $.msg('Parameter missing', 'Please get the parameters first')
    const noJoinExists = APP_IDS.some((app_id) => app_id.split('#')[1] === '0')
    if (!noJoinExists) return $.log('No APP_ID needs to be added')
        ...
        if (status === '0') {
            for (let i = 0; i < LOON_COUNT; i++) {
                ...
                try {
                    ...
                    if (!appData?.data) $.log(`${appId} Unable to accept the invitation, continue execution`)
                    if (appData.data?.status === 'OPEN') {
                        $.log(`${appId}(${appData.data.app.name})`, `Open to join, joining...`)
                        const jsonBody = await TF_Join(appId)
                        $.log(`🎉 Joined successfully`)
                        $.msg(`${jsonBody.data.name}`, '𝐓𝐞𝐬𝐭𝐅𝐥𝐢𝐠𝐡𝐭 joined successfully')
            ...
            $.log('================================')
            $.log(appId + 'Execution completed')
            $.log('================================')
        } else {
            $.log(`${appId} has been added, skip`)
            $.log('================================')
        }
    }
})()
    .catch((e) => $.log('', `❗️${$.name}, error!`, e))
    .finally(() => $.done({}))
// prettier-ignore
