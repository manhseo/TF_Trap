...
if (reg1.test($request.url)) {
  ...
  if ($prefs.valueForKey("request_id") !== null) {
    $notify("TestFlight automatically joins", "Information obtained successfully", "");
  } else {
    $notify("TestFlight automatically joins", "Information acquisition failed", "Please add testflight.apple.com");
  }
  $done({});
  ...
  $notify("TestFlight automatically joins", `Added APP_ID: ${id}`, `Current ID: ${appId}`);
  $done({});
}
